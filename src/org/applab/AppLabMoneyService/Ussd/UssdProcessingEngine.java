...
public class UssdProcessingEngine {
    ...
    public UssdResponse getResponse(UssdRequest request) {
        ...
        try {
            ...
            boolean threadExists = UssdRequestThread.exists(request.transactionId, request.msisdn);

            // If the Thread Exists, then retrieve the Thread
            if (threadExists) {
                // i. get the Request Thread from the Database
                urt = UssdRequestThread.getRequestThread(request.transactionId, request.msisdn);

                // ii. transform the XML String of the ThreadRequests to retrieve the ArrayList of UserInputs
                urt.setThreadRequests(UssdRequestThread.getThreadRequestsFromXml(urt.getXmlThreadHistory()));

                // iii. get the most recent interaction
                String prevPrompt = urt.getPreviousSystemResponse();
                UserInputSystemResponse uisrRecent = urt.getPreviousUserInputSystemResponse();
                ...
                if (uisrRecent != null) {
                    ...
                    // Check whether the previous display was a Menu, Step or Bak_Menu
                    if (uisrRecent.getExtData1().toUpperCase().equalsIgnoreCase("MENU")) {
                        boolean menuItemHasKeyword = false;
                        UssdStepResponse firstStepResp = null;

                        // Get the next menu
                        // First get the actual menuSelection by User
                        String actualItemSelectedByUser = getDisplayItemByUserSelection(prevPrompt, request.userInput);

                        if (actualItemSelectedByUser != null && !actualItemSelectedByUser.isEmpty()) {
                            // Get its MenuItemID
                            ...
                            if (selectedMenuItemId != 0) {
                                ...
                                // Include a procedure to first check whether the selected option should load a sub menu
                                // or start Transaction Steps
                                menuItemHasKeyword = menuItemHasTransKeyword(selectedMenuItemId);

                                // If it has Keyword, then Load the First Step of the Transaction Keyword
                                // OMM: here I get a UssdStepMenu, which extends UssdMenu
                                if (menuItemHasKeyword) {
                                    firstStepResp = getFirstUssdStepMenuResponse(request, selectedMenuItemId);
                                    appResp = firstStepResp.getBaseUssdResponse();

                                    // Build the final Transaction Keyword Thread from the UssdStepResponse
                                    urt.setFinalTransKeyword(firstStepResp.getKeywordCode());

                                    // Check whether the Step has Predefined Input and Flag accordingly
                                    if (firstStepResp.getHasConfirmedPredefInput()) {
                                        externalData2Value = "BNLOOKUP";
                                    }

                                    // Check if the Value for this step is FIXED i.e. preset set extData2 accordingly
                                    // NB: Ensure PRESET takes priority by setting it last
                                    if (firstStepResp.isUseFixedValue()
                                            && (firstStepResp.getFixedValue() != null && !firstStepResp.getFixedValue().isEmpty())) {
                                    ...
                                    appResp = getUssdSubMenu(request, selectedMenuItemId);
                        ...
                        // OMM: I have denormalized some aspects that are from Parent Thread i.e. TransactionID
                        ...
                        UserInputSystemResponses tRequests = urt.getThreadRequests();
                        tRequests.add(uisr);
                        urt.setThreadRequests(tRequests);

                        // Generate the XML Representation of the UssdRequestsThread
                        String xmlThreadHistory = UssdRequestThread.transformRequestThreadToXml(urt);
                        ...
                        // OMM: Strip the Menu Item IDs from the Display String before sending it to Phone
                        appResp.responseToSubscriber = stripMenuItemIdsFromDisplay(appResp.responseToSubscriber);

                        // If the step has a Fixed Response, call this method Recursively
                        // To enter the fixed Value without Prompting the Customer
                        ...
                        if (menuItemHasKeyword) {
                            if (firstStepResp.isUseFixedValue()
                                ...
                                requestFixed.userInput = firstStepResp.getFixedValue();

                                // Call this method again-recursive call and pass the new UssdRequest
                    ...
                    else if (uisrRecent.getExtData1().toUpperCase().equalsIgnoreCase("STEP")) { // If Previous Prompt
                                                                                                // was a STEP and User
                                                                                                // has responded

                        // Get Answered Step Prompt without the Title i.e. the actual Question, but includes
                        // KeywordStepID
                        ...
                        // If the prevPrompt i.e. previous step had a Predefined Input List
                        // then Resolve the selection of the user and get the Actual Option Text e.g. 1. NONE
                        // Treat this like a Menu selection and use same procedure
                        String userInput = "";
                        if (uisrRecent.getExtData2().equalsIgnoreCase("BNLOOKUP")) {
                            String selectedPredef = getDisplayItemByUserSelection(prevPrompt, request.userInput.trim());
                            // Remove the numbering
                            if (selectedPredef.contains(".")) {
                                selectedPredef = selectedPredef.substring(selectedPredef.indexOf("."), selectedPredef.length());
                            }
                            // remove the Input_Item_Id
                            if (selectedPredef.contains("~")) {
                                selectedPredef = selectedPredef.substring(1, selectedPredef.lastIndexOf("~"));
                            }
                        ...
                        // Do some fine-tuning on the user Input before appending it to the Transaction Keyword
                        // i. Replace Spaces with Asterisks or another wild character
                        String refinedUserInput = userInput.trim().replaceAll(" ", "*");

                        // Update the Final Trans Command to include the new item
                        // OMM: These NULLReferencePointer crap are really causing me grief!!!!!!!!!!!!!!!!
                        if (urt.getFinalTransKeyword() != null) {
                            urt.setFinalTransKeyword(urt.getFinalTransKeyword().concat(" ").concat(refinedUserInput));
                        }
                        else {
                        ...
                        UssdStepResponse stepResp = getNextUssdStepMenuResponse(request, answeredStepId);
                        appResp = stepResp.getBaseUssdResponse();

                        // log the response, before committing the same
                        if (appResp.responseToSubscriber.contains("\r\n")) {
                            uisr.setResponseTitle(appResp.responseToSubscriber.substring(0, appResp.responseToSubscriber.indexOf("\r\n")));
                        }
                        else {
                            uisr.setResponseTitle("AppLab Money");
                        }
                        ...
                        // OMM: 2012-02-27
                        // If the Prompt has a Fixed Value, then update ExtData2 to indicate PRESET-VALUE
                        if (stepResp.isUseFixedValue() && (stepResp.getFixedValue() != null && !stepResp.getFixedValue().isEmpty())) {
                        ...
                        // If the Parameter Capture is Complete, then forward the final string to Core Transaction
                        // Processing Engine
                        // TODO: Check if TRANSACTION_KEYWORD Requires Confirmation
                        // I will have an internal servlet that I can just call to invoke the core.
                        ...
                        if (transParamCaptureCompleted) {
                            ...
                            String transCommand = String.format("%s~%s~%s", urt.getTransactionId(), urt.getSourceMsisdn(),
                                    urt.getFinalTransKeyword());
                            // Invoke the Static Method on the RxService
                            AppLabMoneyRxService.processReceivedData(transCommand);
                        }
                    ...
                    else if (uisrRecent.getExtData1().toUpperCase().equalsIgnoreCase("BACK_MENU")) { // If it was
                                                                                                     // BACK_MENU used
                                                                                                     // to Correct User
                                                                                                     // Input
                        // First get the actual menuSelection by User
                        String actualItemSelectedByUser = getDisplayItemByUserSelection(prevPrompt, request.userInput);
                        // If actualItemSelectedByUser is null, then indicate invalid option
                        if (actualItemSelectedByUser != null && !actualItemSelectedByUser.isEmpty()) {
                            // First check whether the selection is to Correction of Invalid Input
                            if (actualItemSelectedByUser.toUpperCase().trim().startsWith("77.")
                                    || actualItemSelectedByUser.toUpperCase().endsWith("BACK")) {
                                UserInputSystemResponse uisrRecentValid = null;
                                // Load the Previous Menu
                                uisrRecentValid = urt.getPreviousValidUserInputSystemResponse();
                                if (uisrRecentValid != null) {

                                    uisrRecentValid.setSequenceNum(urt.getThreadRequests().size() + 1);

                                    // This time assign the response to subscriber from the thread
                                    appResp.responseToSubscriber = uisrRecentValid.getSystemResponse();
                ...
                else { // Otherwise, if there is no previous history then display error
                   ...
                   // Check whether it is the valid USSD Code for AppLab Money
                if (!request.userInput.equalsIgnoreCase(HelperUtils.ROOT_AM_USSD_CODE)) {
                    appResp = new UssdResponse();
                    appResp.responseToSubscriber = HelperUtils.MSG_UNKNOWN_USSD_CODE;
                    // Indicate finished: for lack of a better option am using Charge Info, for now
                    ChargeInfo cInfo = new ChargeInfo();
                    cInfo.contentType = "COMPLETED";
                    appResp.chargeableInfo = cInfo;
                    appResp.isMenu = false;
                ...
                // i. Create it
                urt = new UssdRequestThread(request.transactionId, request.msisdn);
                urt.setAppModule("APPLABMONEY"); // this is mostly on the Dispatcher

                // ii. Capture the first UserInputSystemResponse, usually the root command:
                // I have denormalized some aspects that are from Parent Thread i.e. TransactionID
                uisr = new UserInputSystemResponse();
    ...
    private UssdMenu createRootMenu() {
        ...
        try {
            ...
            while (result.next()) {
                // Build the String containing menuItemName then menuItemId separated by the Tide character
                String menuItem = String.format("%s~%s", result.getString(2), Long.toString(result.getLong(1)));
    ...
    public UssdResponse getRootAppLabMoneyMenu(UssdRequest request) throws Exception {
        ...
        rootMenu.setTitle("AppLab Money");
    ...
    public UssdResponse getUssdSubMenu(UssdRequest request, long parentMenuItemId) throws Exception {
    ...
    /**
     * Get the UssdStepResponse derived from the UssdStepMenu I use UssdStepResponse which extends UssdResponse to get
     * more info Then I will strip it down when sending out to Phone
     */

    public UssdStepResponse getFirstUssdStepMenuResponse(UssdRequest request, long menuItemId) throws Exception {
        ...
        // Determine whether the Step has a Fixed Value specified i.e. No User Response required
        ...
        if (stepMenu.getHasPredefInput() && (stepMenu.getPredefInputValues() != null && !stepMenu.getPredefInputValues().isEmpty())) {
    ...
    private UssdStepMenu createFirstKeywordStep(long menuItemId) {
        ...
        try {
            ...
            PreparedStatement stm = cn.prepareStatement(String.format(sb.toString(), Long.toString(menuItemId)));

            // Execute the Query
            ResultSet result = stm.executeQuery();

            // OMM: There will be only 1 Step in each Displayed Screen, Unlike the Sub-Menus
            while (result.next()) {
                // Build the String containing StepMenuName then keywordStepId separated by the Tide character
                String menuItem = String.format("%s~%s", result.getString(4), Long.toString(result.getInt(3)));
            ...
            if (stepMenu.getHasPredefInput()) {
                stepMenu.setPredefInputValues(getPredefInputItems(stepMenu.getPredefInputId()));
            }
            return stepMenu;
    ...
    /**
     * Get the UssdStepResponse derived from the UssdStepMenu I use UssdStepResponse which extends UssdResponse to get
     * more info Then I will strip it down when sending out to Phone This time the Next ResponseMenu is derived from the
     * current keywordStepId
     */

    public UssdStepResponse getNextUssdStepMenuResponse(UssdRequest request, long currentKeywordStepId) throws Exception {
        ...
        // Remember if last step was expected to be final then mark that the Ussd session terminated abnormally
        if (stepMenu.getKeywordStepId() <= 0) {
            ...
            // OMM: seems isCompleted is not going to client, so I use chargeableInfo
            ChargeInfo cInfo = new ChargeInfo();
            cInfo.contentType = "COMPLETED";
        ...
        // OMM: Just a thought: if it is Step.isLastStep then mark the isCompleted = 1
        isCompleted = 0;
        return appStepResp;
    }

    /**
     * Load the Next Step of the Transaction based on the Current Step ID and Step Number This Assumes that the Steps
     * are sequential starting from 1 increasing by 1
     */
    private UssdStepMenu createNextKeywordStep(long currentKeywordStepId) {
        ...
        try {
            ...
            // OMM: I should find a way of maintaining state of Step Number to reduce on the Sub-Queries
            sb = new StringBuilder();
            sb.append("SELECT TK.KEYWORD_CODE, TK.KEYWORD_NAME, KS.KEYWORD_STEP_ID, KS.STEP_MENU_NAME, KS.STEP_NUMBER, KS.HAS_PREDEF_INPUT,KS.PREDEF_INPUT_ID,KS.USE_FIXED_VALUE,KS.FIXED_VALUE");
            sb.append(" FROM KEYWORD_STEPS KS");
            sb.append(" JOIN TRANSACTION_KEYWORDS TK ON KS.KEYWORD_ID = TK.KEYWORD_ID");
            ...
            PreparedStatement stm = cn.prepareStatement(String.format(sb.toString(), Long.toString(currentKeywordStepId),
    ...
    private ArrayList<String> getPredefInputItems(long predefInputId) {
        ...
        try {
            ...
            PreparedStatement stm = cn.prepareStatement(String.format(sb.toString(), Long.toString(predefInputId)));
            ...
            // OMM: There will be only 1 Step in each Displayed Screen, Unlike the Sub-Menus
            while (result.next()) {
                // Build the String containing PredefInputItemCode then PredefInputItemId separated by the Tide
                // character
                String menuItem = String.format("%s~%s", result.getString(2), Long.toString(result.getInt(1)));
    ...
    /**
     * This procedure assumes that the MenuItemID is appended at the end of the MenuText
     */
    private String stripMenuItemIdsFromDisplay(String theDisplayString) {
        ...
        try {
            ...
            for (int i = 0; i < displayItems.length; i++) {
                ...
                // If there is no Tide character do not substring, to avoid -1 as index
                if (displayItems[i].contains("~")) {

                    // Truncate the last part of the string
                    displayItems[i] = displayItems[i].substring(0, displayItems[i].lastIndexOf("~"));
     ...
     */
    private String getDisplayItemByUserSelection(String previousPrompt, String userSelection) {
        ...
        try {
            ...
            for (int i = 0; i < displayItems.length; i++) {
                if (displayItems[i].trim().toUpperCase().startsWith(userSelection.toUpperCase())) {
    ...
    private long getSelectedMenuItemId(String selectedMenuItem) {
        ...
        try {
            ...
            if (selectedMenuItem.contains("~")) {
                String lastPart = selectedMenuItem.substring(selectedMenuItem.lastIndexOf("~") + 1);
    ...
    private boolean menuItemHasTransKeyword(long menuItemId) {
        ...
        try {
            ...
            if (HelperUtils.TARGET_DATABASE.equalsIgnoreCase("MYSQL")) {
                stm = cn.prepareStatement(String.format(
                        "SELECT IFNULL(KEYWORD_ID,-999) KEYWORD_ID FROM USSD_MENU_ITEMS WHERE MENU_ITEM_ID = %s", Long.toString(menuItemId)));
            }
            else if (HelperUtils.TARGET_DATABASE.equalsIgnoreCase("ORACLE")) {
                stm = cn.prepareStatement(String.format(
                        "SELECT NVL(KEYWORD_ID,-999) KEYWORD_ID FROM USSD_MENU_ITEMS WHERE MENU_ITEM_ID = %s", Long.toString(menuItemId)));
            }
            else { // Assume SQL Server or die
                stm = cn.prepareStatement(String.format(
                        "SELECT ISNULL(KEYWORD_ID,-999) KEYWORD_ID FROM USSD_MENU_ITEMS WHERE MENU_ITEM_ID = %s", Long.toString(menuItemId)));
            }
            ...
            // if there is a KeywordId then it is linked to a Transaction Keyword
            if (keywordId > 0) {
                return true;
            }
            else {
    ...
    /**
     * Get the keywordStepId for the Previous Prompt that the User has responded to.
     */
    private long getAnsweredKeywordStepId(String previousStepPrompt) {
        ...
        try {
            ...
            if (previousStepPrompt.contains("~")) {
                String lastPart = previousStepPrompt.substring(previousStepPrompt.lastIndexOf("~") + 1);
    ...
    /**
     * Get the Step Prompt that was to the user for which answer we have just received Important when the Title is
     * displayed together with Prompt
     */
            ...
            if (request != null) {
                if (request.msisdn != null && !request.msisdn.isEmpty()) {
                    if (request.transactionId != null && !request.transactionId.isEmpty()) {
                        if (request.userInput != null && !request.userInput.isEmpty()) {
                            isValid = true;
                        }
                    }
                }
    ...
    /***
     * This Procedure determines which Menu Options to Exclude in the displayed set of USSD Menu
    ...
    private ArrayList<Integer> getMenuItemsToExclude(String sourceMsisdn) {        
        ...
        try { 
            ...
            //If the Customer is Not Registered or is not a Dealer or Agent, then hide the Dealer Menus
            if(!showDealerMenu(sourceMsisdn)) {
                if(keywordsToExclude.isEmpty()) {
                    keywordsToExclude = keywordsToExclude.concat("'KYCR','DPST'");   
                }
                else {
                    keywordsToExclude = keywordsToExclude.concat(",").concat("'KYCR','DPST'"); 
                }                    
            }          
            
            //Check whether to hide Me2Me Activation
            if(org.applab.AppLabMoneyCore.Me2Me.MeToMeCommon.checkMeToMeActivation(sourceMsisdn)) {
                
                if(keywordsToExclude.isEmpty()) {
                    keywordsToExclude = keywordsToExclude.concat("'ACTV'");   
                }
                else {
                    keywordsToExclude = keywordsToExclude.concat(",").concat("'ACTV'"); 
                }
            }
            else {
                //Exclude all the rest of the Me2Me Keywords
                if(keywordsToExclude.isEmpty()) {
                    keywordsToExclude = keywordsToExclude.concat("'CRGL','MTOM','GBAL','TENQ','REDM','REBT','STOP'");   
                }
                else {
                    keywordsToExclude = keywordsToExclude.concat(",").concat("'CRGL','MTOM','GBAL','TENQ','REDM','REBT','STOP'"); 
                }
            }
    ...
    private boolean showDealerMenu(String targetMsisdn) {
        ...
        try {
            CustomerInformation sourceCustInfo = CustomerInformation.getCustomerAccountInfo(targetMsisdn);
    ...
    private ArrayList<Integer> getMenuItemIdsToExclude(String keywordsToExclude) {
        ...
        try {
            ...
            sb.append(" WHERE UPPER(TK.KEYWORD_CODE) IN (%s)");
            String query = sb.toString();

            PreparedStatement stm = cn
                    .prepareStatement(String.format(query, keywordsToExclude.toUpperCase()));
